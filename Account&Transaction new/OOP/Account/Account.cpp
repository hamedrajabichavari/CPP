...
bool Account::deposit(double amount , int type){
    ...
    if(amount > 1000 ){
        std::cout << "Message : amount should be less than 1000 CAD " << std::endl;
        return false;
    }
...
bool Account::withdrawl(double amount, int type){
    if(amount >= this->balance.at(type) ){
        std::cout << "There is only "<< this->balance.at(type) << " CAD in your account" << std::endl;
        std::cout << "Transaction canceled"<< std::endl;
        return false;
    }
    if(amount <= 0 ){
        std::cout << "You should withdrawl greater than 0" << std::endl;
        std::cout << "Transaction canceled"<< std::endl;
        return false;
    }
